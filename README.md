...
My advice is to work slowly. Look at our [implementation](https://github.com/Georgia-Tech-Off-Road/CMBTL_DAQInfrastructure) for inspiration. My advice is to start by defining your first sensor in a seperate header file, in our other [repo](https://github.com/Georgia-Tech-Off-Road/CMBTL_DAQInfrastructure) look at src/Sensors for examples. Then in your equivalent of DAQSensors.h, define (with ``using``) a type of ``std::tuple<YourNewSensorClassType>``. This is metaprogramming, anything involved must be known at compile time. That is why we define types instead of declaring variables.  Define another type (corresponding with ``DAQSensorDataType`` in src/DAQInfrastructure/DAQSensors.h) of type ``SensorData<MyTupleTypeDefinedInThePreviousStep>``. Now COMPILE! If this works, add another sensor, rinse, wash, and repeat. 
...
# Testing
We test all of our libraries in our [main repository](https://github.com/Georgia-Tech-Off-Road/GTOR-DAQ). Please not that most of the useful testing is found in our implementation of this infrastructure library, found [here](https://github.com/Georgia-Tech-Off-Road/CMBTL_DAQInfrastructure). This is our actual up-to date code that we use for our vehicle's sensor configuration. However, if you would like to test this library locally, please download our version of [BoostForArduino](https://github.com/Georgia-Tech-Off-Road/BoostForArduino). You will then need to add Add BoostForArduino's src dir to this library's include path. We already reference a library named BoostForArduino in this repo's CMakeLists.txt to work with our [main repository](https://github.com/Georgia-Tech-Off-Road/GTOR-DAQ). If you would like to test or use this library, these steps below will help you get started. 
